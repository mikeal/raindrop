...
function (rd, dojo) {
    ...
    api.extend({
         ...
         */
        put: function (args) {
            var doc = this.newDoc(args.doc), docUrl, xhrArgs;
            ...
            docUrl = this.dbPath(args) + doc._id;
            if (doc._rev) {
                docUrl += "?rev=" + doc._rev;
            }
...
*/
        ...
        newDoc: function (doc) {
            ...
            //Make sure schema setup fields are on there correctly for it to
            //participate in the back end extensions.
            var schemaItems = {}, newSchema;
            
            if (!doc.rd_schema_provider) {
                doc.rd_schema_provider = rd.uiExtId;
            }
  
            if (!doc.rd_schema_items) {
                doc.rd_schema_items = {};
                doc.rd_schema_items[doc.rd_schema_provider] = {
                    rd_source: null,
                    schema: null
                };
            }
